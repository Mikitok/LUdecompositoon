...
[vec1, ~] = tdfda1(X_svd1, max(Y_trn1)) ;

% %%%%%  使用生成的新的训练样本集分类%%%%%%
% for i=1:numtrn*2
% %     X_trn1{i}=vec1(:,1:k)'*double(X_svd1{i});
...
ratel1(4,t+1)=mean(out==Y_tst1);

[X_svd2,Y_svd2]=svddecomposition(X_trn2,Y_trn2);
% [vec2, ~] = tdfda(X_svd2, max(Y_trn2)) ;
[vec2, ~] = tdfda1(X_svd2, max(Y_trn2)) ;
%%%%%  使用生成的新的训练样本集分类%%%%%%
% for i=1:numtrn*2
% %     X_trn2{i}=vec2(:,1:k)'*double(X_svd2{i});
